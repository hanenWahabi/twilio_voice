...
class TwilioVoice {
  ...
  set showReturnCallOptionInNotifications(bool value) {
    _channel.invokeMethod(
        'show-return-call-option', <String, dynamic>{"show": value});
  }
  ...
  CallEvent _parseCallEvent(String state) {
    if (state.startsWith("DEVICETOKEN|")) {
      ...
      // source: https://www.twilio.com/docs/api/errors/31603, https://jp.twilio.com/docs/api/errors/31486
      // The callee does not wish to participate in the call.
      if (tokens[1].contains("31603")) {
...
ActiveCall createCallFromState(String state,
    {CallDirection? callDirection, bool initiated = false}) {
  List<String> tokens = state.split('|');
  final params = parseCustomParams(tokens);
  return ActiveCall(
      from: tokens[1],
      to: tokens[2],
      initiated: initiated ? DateTime.now() : null,
...
Map<String, dynamic>? parseCustomParams(List<String> tokens) {
  try {
    Map<String, dynamic> customValue = jsonDecode(tokens.last);
    return customValue;
  } catch (error) {
    return null;
  }
